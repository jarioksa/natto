...
$N_1 = \exp(H')$, $N_2 = D_2$, and $N_\infty = 1/(\max p_i)$. The
...
species, and the beta diversity $\beta$ for a cluster of $N$ sampling
units is defined as
\begin{equation}
  \label{eq:partition}
  \beta_a = \underbrace{H_a \left(\sum_{i=1}^N x_{ij}
    \right)}_{\gamma} - \underbrace{\frac{1}{N} \sum_{i=1}^N H_a
    (x_{ij})}_{\bar \alpha}\,,
\end{equation}
...
differences. This really seem to be the case with Renyi diversities,
...
equalization is dependent on the scale $a$ of the Renyi diversity, and
...
matrix. The information content $I$ of a cluster of $M$ sites is
defined as \cite{WilliamsEa66, LanceWilliams66}: 
\begin{multline}
...
This defines a community with maximum equitability, so that all Renyi
