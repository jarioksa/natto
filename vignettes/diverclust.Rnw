...
diversity indices are special cases of Hill numbers: $N_0 = S$ is
number of species, $N_1 = \exp(H')$ is exponent of Shannon diversity,
$N_2 = 1/\sum_{j=1}^S p_j^2$ is the Simpson diversity, and
...
first includes both the within-unit diversity and the between-unit
differences and represents the overall diversity also known as the
gamma diversity, and the second term is the averaged within-site
diversity, and the equation impl9es additive
partitioning of diversity. However, Rényi diversities are on $\log$
scale which translates a multiplicative model into an additive
...
information content of single sampling units is always $I=0$, but
formed clusters have positive values of $I$, and the information
values of the components of clusters are subtracted from the value of
the cluster, and the difference $\Delta I$ is used as the criterion of
clustering. The merge still happens at the level of pooled new unit
$I$, and the clusters are not formed in the order of their merge
heights. This conflicts with \R{} conventions, and the
\code{infoclust} function updates the merge table to correspond to the
merge heights.
...
@ 
The function uses only presence-absence information. The clusters are
often very compact (Fig.~\ref{fig:infoclust}).
...
because the indices cannot be directly updated to dissimilarities
between clusters: the original data must be update by pooling sampling
units or cluster after each merge, and re-evaluating the
dissimilarities from updated community data. However, it may be
instructive to compare these indices with common dissimilarity
measures. In this chapter we see how the diversity measures can be
expressed as dissimilarities with binary data.
...
alpha diversity for two sampling units with $S_1$ and $S_2$ species is
\begin{align}
  \label{eq:alphaH}
...
of beta diversity for two binary sampling units with Rényi diversity
(eq.~\ref{eq:renyi}) is
\begin{multline}
...
importantly, they are intended for comparisons among multiple sampling
units, and this can only be done via pooling original sampling units.

The implicit dissimilarity measure in information analysis is
\begin{equation}
  d(I) = (S_1 + S_2 - 2J) \log(4) \,.
\end{equation}
This is $d(N_0)$ with different scaling. However, the similarity to
